...
namespace Quang.RedisCache
{
    public class RedisCacheClient : ICacheClient
    {
        ...
        public async Task<bool> AddAsync<T>(string key, T value) where T : class
        {
            ...
            return await db.StringSetAsync(key, entryBytes, flags: CommandFlags.FireAndForget);
        }
        ...
        {
            var redisInput = items.Select(item => new KeyValuePair<RedisKey, RedisValue>(item.Item1, serializer.Serialize(item.Item2))).ToArray();
            var result = await db.StringSetAsync(redisInput, When.Always, CommandFlags.FireAndForget);
